...
class KDACommandLineLauncher : SunCommandLineLauncher {
    ...
    override fun description(): String? {
        return "A custom launcher supporting cwd"
    }

    /**
     * Copied from SunCommandLineLauncher.java and added cwd processing logic
     */
    ...
    override fun launch(arguments: Map<String, Connector.Argument>): VirtualMachine {
        ...
        var cwd = argument(ARG_CWD, arguments).value()
        var exePath: String?
        if (quote.length > 1) {
        ...
        try {
            ...
            vm = launch(commandArray = tokenizeCommand(command, quote[0]), listenKey = listenKey,
                    ts = transportService(), cwd = cwd, grp = grp
            )
        } finally {
    ...
    fun launch(commandArray: Array<String>,
                        ...
                        ts: TransportService, cwd: String?, grp: ThreadGroup): VirtualMachine {
        val helper = Helper(commandArray, listenKey, ts, cwd, grp)
    ...
    class Helper internal constructor(private val commandArray: Array<String>, private val listenKey: TransportService.ListenKey,
                                      private val ts: TransportService, private val cwd: String? = null, private val grp: ThreadGroup) {
        ...
        fun launchAndAccept() {
            synchronized(lock) {
                process = Runtime.getRuntime().exec(commandArray, null, cwd?.let { File(it) })
